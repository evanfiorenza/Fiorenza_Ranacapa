...
server <- function(input, output,session)({
  ...
  output$biomSelect <- renderUI({
    ...
    if (input$mode == "Custom") {
      fileInput("in_taxon_table", "Please select your taxonomy table.
                Note: this should be saved either as *.biom, *.csv, *.tsv, or *.txt",
                accept = c(".biom", ".txt", ".tsv",".csv"))
    }
  })
  output$metaSelect <- renderUI({
    ...
    if (input$mode == "Custom") {
      fileInput("in_metadata", "Please select the metadata file.
                Note: this should be saved as *.csv, *.txt or *.tsv",
 ...
 output$which_palette <- renderUI({
   gg_color_hue <- function(n) {
     hues = seq(15, 375, length = n + 1)
     hcl(h = hues, l = 65, c = 100)[1:n]
   }
   if (input$rared_taxplots == "unrarefied") {
     zz <- data.frame(sample_data(data_subset_unrare()))
   } else {
     zz <- data.frame(sample_data(data_subset()))
   }
   lev <- sort(unique(zz[,input$var])) # sorting so that "things" are unambigious
   cols <- gg_color_hue(length(lev))

   # New IDs "colX1" so that it partly coincide with input$select...
   lapply(seq_along(lev), function(i) {
     colourpicker::colourInput(inputId = paste0("col", gsub(" ","",lev[i])),
  ...
  taxonomy_table <- reactive({
    if (input$mode == "Custom") {
      ...
      else if (grepl(input$in_taxon_table$datapath, pattern = ".csv")) {
        read.csv(input$in_taxon_table$datapath) %>%
          scrub_seqNum_column() %>%
          scrub_taxon_paths () %>%
          group_anacapa_by_taxonomy()
      }
    ...
    if (input$mode == "Custom") {
      if (grepl(readLines(input$in_metadata$datapath, n = 1), pattern = "^#")) {
        phyloseq::import_qiime_sample_data(input$in_metadata$datapath) %>%
          as.matrix() %>%
          as.data.frame()%>%mutate(across(where(is.numeric),as.factor))
      } else if(grepl(input$in_taxon_table$datapath, pattern = ".txt") |
                grepl(input$in_taxon_table$datapath, pattern = ".tsv")){
        read.table(input$in_metadata$datapath,
                   header = 1, sep = "\t", stringsAsFactors = F,
                   quote = "", comment.char = "")%>%mutate(across(where(is.numeric),as.factor))
      }
      else if(grepl(input$in_taxon_table$datapath, pattern = ".csv")){
        read.table(input$in_metadata$datapath,
                   header = 1, sep = ",", stringsAsFactors = F,
  ...
  output$alpharichness <- renderPlotly({
    ...
    withProgress(message = 'Rendering alpha diversity plot', value = 0, {
      ...
      cols <- paste0("c(", paste0("input$col", sort(gsub(" ","",unique(zz[,input$var]))), collapse = ", "), ")")
      # print(cols)
      cols <- eval(parse(text = cols))
      req(length(cols) == length(unique(zz[,input$var])))
  ...
  output$betanmdsplotly <- renderPlotly({
    withProgress(message = 'Rendering beta diversity plot', value = 0, {
      ...
      cols <- eval(parse(text = cols))
      req(length(cols) == length(unique(zz[,input$var])))
      nmdsplot <- plot_ordination(data_subset(), ord, type="samples",
                                  color = input$var) +
  ...
  output$adonisTable <- renderTable ({
    sampledf <- data.frame(sample_data(data_subset_unrare()))
    dist_obj <- phyloseq::distance(data_subset_unrare(), method = dissimMethod())
    broom::tidy(adonis2(as.formula(paste("dist_obj~", input$var)), data = sampledf))

  }, digits = 5)

  output$permTestTable <- renderPrint({
    sdf <- as(sample_data(data_subset_unrare()), "data.frame")
    dist_obj <- phyloseq::distance(data_subset_unrare(), method = dissimMethod())
    betadisper(dist_obj, getElement(sdf, input$var))
  })

  output$permTestAOV <- renderTable({
    sdf <- as(sample_data(data_subset_unrare()), "data.frame")
    dist_obj <- phyloseq::distance(data_subset_unrare(), method = dissimMethod())
    disp=betadisper(dist_obj, getElement(sdf, input$var))
    broom::tidy(anova(disp))
  },digits=5)
  ...
  #Add text and  relative abundance plot
  output$tax_bar <- renderPlotly({
    ...
    withProgress(message = 'Rendering taxonomy barplot', value = 0, {
      ...
      if(input$var =="Sample"){
      p=plot_bar(physeqGlommed, fill = input$taxon_level) + theme_ranacapa() +
        theme(axis.text.x = element_text(angle = 45)) +
        theme(axis.title = element_blank())
      p=p+geom_col(color=NA,position='stack')
      }
      else{
        p=plot_bar(physeqGlommed, fill = input$taxon_level) + theme_ranacapa() +
          theme(axis.text.x = element_text(angle = 45)) +
          theme(axis.title = element_blank())+
          facet_wrap(as.formula(paste("~", input$var)),scales = "free_x")
        p=p+geom_col(color=NA,position='stack')
      }
      gp <- ggplotly(p) %>%
        layout(yaxis = list(title = "Number of Reads", titlefont = list(size = 16)),
  ...
  output$rel_bar <- renderPlotly({

    withProgress(message = 'Rendering taxonomy barplot', value = 0, {
      incProgress(0.5)

      if (input$rared_taxplots == "unrarefied") {
        physeqGlommed = tax_glom(transform_sample_counts(data_subset_unrare(),function(x) 100 * x/sum(x)), input$taxon_level)
      } else {
        physeqGlommed = tax_glom(transform_sample_counts(data_subset(),function(x) 100 * x/sum(x)), input$taxon_level)
      }
      ...
      gp <- ggplotly(p) %>%
        layout(yaxis = list(title = "Relative Read Abundance (%)", titlefont = list(size = 16)),
               xaxis = list(title = "Sample", titlefont = list(size = 16)),
               margin = list(l = 70, b = 100))
      gp
    })
  ...
  for_hm <- reactive({
    ...
    for_hm <- for_hm %>%
      group_by(across(all_of(input$taxon_level))) %>%
      # group_by(Species) %>%
      summarize(across(where(is.numeric), sum))


    #for_hm <- for_hm[which(rowSums(dplyr::select(for_hm,-one_of(input$taxon_level))) > 0),]
    for_hm[for_hm == 0] <- NA
    tx<-for_hm[[input$taxon_level]]
    ts<-colnames(dplyr::select(for_hm,-one_of(input$taxon_level)))
    for_hm<-data.frame(t(select(for_hm,-one_of(input$taxon_level))))
    colnames(for_hm)<-tx
    for_hm$Sample<-ts

    for_hm<-left_join(for_hm,zz)%>%
    group_by(across(all_of(input$var))) %>%
      summarize(across(any_of(tx), ~mean(.x,na.rm=T)))

    tv<-for_hm[[input$var]]
    for_hm<-data.frame(t(select(for_hm,one_of(tx))))

    colnames(for_hm)<-tv
    for_hm$Taxa<-tx
    rownames(for_hm) <- NULL
    for_hm<-for_hm%>%data.frame%>%column_to_rownames("Taxa")

    #data.frame #%>%
      #column_to_rownames("get.input.taxon_level.")
        # column_to_rownames("Species")
    #row.names(for_hm)<-for_hm%>%select(input$taxon_level)
    for_hm
  })
  output$tax_heat <- renderPlotly({
    ...
    withProgress(message = 'Rendering taxonomy heatmap', value = 0, {
      ...
      selected_taxa <- for_hm()%>%row.names()
      } else{
